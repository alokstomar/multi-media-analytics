...
// payload shape, fetch-based _chatCompletions transport) and only override
// the constructor: swap the baseURL, change the default models to Llama 3,
// and re-skin the provider labels used in logs / cache entries / healthCheck
// so Groq runs are distinguishable.
//
// Cost-estimation caveat: OpenAIProvider calls calculateCost() that looks up
// the MODEL_PRICING table. Groq model names match the 'groq' section there;
// unknown names fall back to gpt-4o-mini pricing. AIUsageLog entries remain
// valid for trend tracking; only absolute $ figures are approximate.

export class GroqProvider extends OpenAIProvider {
  constructor(apiKey) {
    super(apiKey)

    // Override the transport target — _chatCompletions() reads this.baseURL.
    ...
    // Budget gates (separate from DeepSeek's so operators can cap Groq spend).
