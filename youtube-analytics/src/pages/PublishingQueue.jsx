...
export default function PublishingQueue() {
  ...
  const loadJobs = async (silent = false) => {
    ...
    try {
      const res = await getSchedulerJobs({ platform: 'twitter' })
      if (res?.success) {
        setJobs(res.data || [])
      } else {
        setJobs([])
      ...
      console.error('Failed to load Twitter jobs:', err)
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-gray-100 text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                  <th className="pb-3.5 pl-2">Tweet / Post Content</th>
      ...
      <div className="bg-indigo-50/20 border border-indigo-100/50 rounded-2xl p-5 shadow-inner flex gap-3.5 items-start">
        ...
        <div className="space-y-1">
          ...
          <p className="text-[10px] text-indigo-700 leading-relaxed font-medium">
            Publishing retries use an exponential backoff timing trigger. If a network block persists after 3 retry iterations, jobs are automatically marked as failed to prevent Twitter Graph API spam locks.
          </p>
        </div>
      </div>
    </div>
