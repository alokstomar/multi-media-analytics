...
function generateFallback(channelData, platform) {
  ...
  return Array.from({ length: 3 }, (_, i) => {
    ...
    return {
      id: i + 1,
      title: `How to make your next ${platform === 'instagram' ? 'Reel' : 'Short'} viral (${baseTitle.slice(0, 20)}...)`,
      hook: `Start with a 1.5s high-contrast question matching "${baseTitle.slice(0, 30)}..." to lock user attention.`,
      first3s: 'Immediate action sequence without a boring introduction.',
      cta: platform === 'instagram' ? 'Hit follow for daily creator intelligence reports!' : 'Hit subscribe for daily creator intelligence reports!',
      retention: `${Math.round(85 + ((i * 7) % 30))}%`,
      viralScore: Math.round(80 + ((i * 4) % 19)),
      trendStrength: Math.round(75 + ((i * 3) % 23)),
    }
  })
}

export default function ShortsIdeasSection({ channelData }) {
  const [isOpen, setIsOpen] = useState(true)
  const { selectedPlatform } = usePlatform()
  const { activeAccountId: activeChannelId } = usePlatformAdapter()
  const [shorts, setShorts] = useState(() => generateFallback(channelData, selectedPlatform))

  useEffect(() => {
    if (!activeChannelId || activeChannelId === 'demo' || activeChannelId === 'demo_ig') {
      setShorts(generateFallback(channelData, selectedPlatform))
      return
    }
    apiGenerateShortsIdeas(activeChannelId)
      .then((res) => {
      ...
      .catch(() => setShorts(generateFallback(channelData, selectedPlatform)))
    ...
    <div className="rounded-[20px] border border-gray-100 bg-white overflow-hidden" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.03), 0 4px 16px -4px rgba(0,0,0,0.06)' }}>
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="p-6 pt-3 border-t border-gray-50 grid grid-cols-1 md:grid-cols-3 gap-5 bg-gray-50">
              {shorts.map((item, idx) => (
                <motion.div
                  key={item.id}
